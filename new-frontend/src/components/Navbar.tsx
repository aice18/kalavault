    ...
    <header className="fixed top-0 inset-x-0 z-50 border-b border-white/10 bg-transparent backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-6 py-5 md:px-8">
        ...
        <nav className="hidden items-center gap-8 text-sm uppercase tracking-[0.2em] text-[#d6b771] md:flex">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="transition hover:text-[#f3e0b1]">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/login" className="hidden rounded-none border border-[#d6b771] bg-transparent px-6 py-3 text-sm uppercase tracking-[0.18em] text-[#d6b771] transition hover:border-[#f3e0b71a] hover:text-[#f3e0b1] md:inline-flex">
            Login
          </Link>
        </div>
      </div>
